...
const props = withDefaults(
  defineProps<{
    ...
    expression?: string
    /** taille des traits / yeux du visage (small, medium, large) */
    featureSize?: FeatureSize
    /** style des sourcils (none, arched, straight, dots) */
    eyebrows?: EyebrowsStyle
    /** style du nez (none, dot, pill, snout) */
    nose?: NoseStyle
    /** style des joues / blush (none, blush, dots, lines) */
    cheeks?: CheeksStyle
    /** style de la bouche (none, smile, grin, frown, open, cat, straight) */
    mouth?: MouthStyle
...
const ink = computed(
  () =>
    COLOR_BY_ID.get(props.color)?.hex ??
...
const engine = new BotEngine(
  ...
  shapeRadii.value,
  expression.value,
  props.featureSize ?? DEFAULT_FEATURE_SIZE,
  props.eyebrows ?? DEFAULT_EYEBROWS,
  props.nose ?? DEFAULT_NOSE,
  props.cheeks ?? DEFAULT_CHEEKS,
  props.mouth ?? DEFAULT_MOUTH
)
...
watch(
  () => props.featureSize,
  (size) => {
    engine.setFeatureSize(size ?? DEFAULT_FEATURE_SIZE, clock)
    redrawFrozen()
  }
)
...
watch(
  () => props.eyebrows,
  (val) => {
    engine.setEyebrows(val ?? DEFAULT_EYEBROWS)
    redrawFrozen()
  }
)
...
watch(
  () => props.nose,
  (val) => {
    engine.setNose(val ?? DEFAULT_NOSE)
    redrawFrozen()
  }
)
...
watch(
  () => props.cheeks,
  (val) => {
    engine.setCheeks(val ?? DEFAULT_CHEEKS)
    redrawFrozen()
  }
)
...
watch(
  () => props.mouth,
  (val) => {
    engine.setMouth(val ?? DEFAULT_MOUTH)
    redrawFrozen()
  }
)
