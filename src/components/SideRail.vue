...
export type ViewId = 'personnaliser' | 'animations' | 'combos' | 'reel' | 'lab3d' | 'reglages'
...
const ITEMS = computed<Array<{ id: ViewId; label: string }>>(() => [
  ...
  { id: 'combos', label: t('rail.combos') },
  { id: 'reel', label: t('rail.reel') },
  { id: 'lab3d', label: t('rail.lab3d') },
...
<template>
  <nav
    class="fixed top-3 left-1/2 z-20 -translate-x-1/2 rounded-2xl border border-[var(--line)] bg-white/85 p-1.5 shadow-sm backdrop-blur lg:top-1/2 lg:left-4 lg:translate-x-0 lg:-translate-y-1/2 dark:bg-zinc-900/85"
    ...
    <ul class="flex gap-1 lg:flex-col">
      ...
      >
        ...
        >
          ...
          <!-- Combos : Lightning Star / Magic Sparks -->
          <svg
          ...
          >
            <path
              fill="currentColor"
              d="M13 2L3 14h8l-2 8l10-12h-8l2-8z"
            />
          </svg>
