...
const color = ref(
  stored('couleur', DEFAULT_COLOR, (v) => COLOR_BY_ID.has(v) || /^#([0-9a-fA-F]{3,8})$/.test(v))
)
const expression = ref(
  stored('expression', DEFAULT_EXPRESSION, (v) => EXPRESSION_BY_ID.has(v))
)
const featureSize = ref<FeatureSize>(
  stored(
    'taille_traits',
    DEFAULT_FEATURE_SIZE,
...
const eyebrows = ref<EyebrowsStyle>(
  stored('sourcils', DEFAULT_EYEBROWS, (v) =>
    ['none', 'arched', 'straight', 'dots'].includes(v)
  ) as EyebrowsStyle
)
const nose = ref<NoseStyle>(
  stored('nez', DEFAULT_NOSE, (v) => ['none', 'dot', 'pill', 'snout'].includes(v)) as NoseStyle
)
const cheeks = ref<CheeksStyle>(
  stored('joues', DEFAULT_CHEEKS, (v) =>
    ['none', 'blush', 'dots', 'lines'].includes(v)
  ) as CheeksStyle
)
const mouth = ref<MouthStyle>(
  stored('bouche', DEFAULT_MOUTH, (v) =>
...
watch(featureSize, (v) => ecris('taille_traits', v))
watch(eyebrows, (v) => ecris('sourcils', v))
watch(nose, (v) => ecris('nez', v))
      ...
      >
        ...
        <template v-else>
          <Customizer
            ...
            v-model:feature-size="featureSize"
            v-model:eyebrows="eyebrows"
            v-model:nose="nose"
