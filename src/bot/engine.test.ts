...
describe('etats', () => {
   ...
   */
  it('garde tous les etats dans la sequence (14 classiques + 8 personnalites)', () => {
    expect(SEQUENCE).toHaveLength(22)
    expect(new Set(SEQUENCE).size).toBe(22)
...
describe('changement d etat pendant un fondu', () => {
  ...
  it('adapte la taille des yeux selon featureSize (small / medium / large)', () => {
    const eMedium = new BotEngine(100, 'idle', null, null, 'medium')
    const eSmall = new BotEngine(100, 'idle', null, null, 'small')
    const eLarge = new BotEngine(100, 'idle', null, null, 'large')

    const fMedium = eMedium.sample(1)
    const fSmall = eSmall.sample(1)
    const fLarge = eLarge.sample(1)

    expect(fMedium.eyes).toHaveLength(2)
    expect(fSmall.eyes).toHaveLength(2)
    expect(fLarge.eyes).toHaveLength(2)

    // La longueur du chemin SVG de l'œil reflète la taille de la gélule
    // On peut aussi vérifier que setFeatureSize modifie le rendu
    const eDynamic = new BotEngine(100, 'idle')
    const f1 = eDynamic.sample(1)
    eDynamic.setFeatureSize('small', 1)
    const f2 = eDynamic.sample(2) // après le morph
    expect(f2.eyes[0]!.d).not.toBe(f1.eyes[0]!.d)
  })

  it('génère les sourcils, le nez, les joues et la bouche quand ils sont configurés', () => {
    const eNone = new BotEngine(100, 'idle', null, null, 'medium', 'none', 'none', 'none', 'none')
    const fNone = eNone.sample(1)
    expect(fNone.eyebrows).toHaveLength(0)
    expect(fNone.nose).toHaveLength(0)
    expect(fNone.cheeks).toHaveLength(0)
    expect(fNone.mouth).toHaveLength(0)

    const eAll = new BotEngine(100, 'idle', null, null, 'medium', 'arched', 'dot', 'blush', 'smile')
    const fAll = eAll.sample(1)
    expect(fAll.eyebrows).toHaveLength(2)
    expect(fAll.nose).toHaveLength(1)
    expect(fAll.cheeks).toHaveLength(2)
    expect(fAll.mouth).toHaveLength(1)

    // Modification dynamique
    eNone.setEyebrows('straight')
    eNone.setNose('snout')
    eNone.setCheeks('lines')
    eNone.setMouth('grin')
    const fDynamic = eNone.sample(1)
    expect(fDynamic.eyebrows).toHaveLength(2)
    expect(fDynamic.nose).toHaveLength(1)
    expect(fDynamic.cheeks).toHaveLength(2)
    expect(fDynamic.mouth).toHaveLength(1)
  })
})
