...
export interface RenderedFaceFeature {
  d: string
  matrix: string
  color?: string
  stroke?: string
...
export interface BotFrame {
  ...
  bodyAlpha: number
  eyes: RenderedEye[]
  eyebrows: RenderedEye[]
  nose: RenderedEye[]
  cheeks: RenderedFaceFeature[]
  mouth: RenderedEye[]
  dots: DotRender[]
...
export class BotEngine {
  ...
  private exprAt = -10
  private featureSize: FeatureSize = 'medium'
  private featureSizePrev: FeatureSize = 'medium'
  private featureSizeAt = -10
  private eyebrows: EyebrowsStyle = 'none'
  private nose: NoseStyle = 'none'
  private cheeks: CheeksStyle = 'none'
  private mouth: MouthStyle = 'none'
  ...
  constructor(
    ...
    eyebrows: EyebrowsStyle = 'none',
    nose: NoseStyle = 'none',
    cheeks: CheeksStyle = 'none',
    mouth: MouthStyle = 'none'
  ) {
    this.scale = scale
    this.cur = initial
    this.shape = shape
  ...
  /**
   * Éléments du visage choisis dans le personnalisateur (sourcils, nez, joues, bouche).
   */
  setEyebrows(style: EyebrowsStyle = 'none') {
    this.eyebrows = style
  }

  setNose(style: NoseStyle = 'none') {
    this.nose = style
  }

  setCheeks(style: CheeksStyle = 'none') {
    this.cheeks = style
  }

  setMouth(style: MouthStyle = 'none') {
    this.mouth = style
  }

  /**
   * Taille des traits / yeux choisie dans le personnalisateur (small, medium, large).
   */
  setFeatureSize(size: FeatureSize = 'medium', now = 0) {
    if (size === this.featureSize) return
    this.featureSizePrev = this.featureSize
    this.featureSize = size
    this.featureSizeAt = now
  }

  /** Échelle des traits effective a l'instant `now`, morph en cours compris. */
  private featureScaleAtTime(now: number): number {
    const to = FEATURE_SIZE_SCALES[this.featureSize] ?? 1
    const from = FEATURE_SIZE_SCALES[this.featureSizePrev] ?? 1
    if (to === from) return to
    const k = (now - this.featureSizeAt) / BotEngine.SHAPE_MORPH
    if (k >= 1) return to
    return lerp(from, to, easings.easeOutQuint(clamp(k)))
  }
  ...
  sample(now: number): BotFrame {
    ...
    const fScale = this.featureScaleAtTime(now)

    const eyes: RenderedEye[] = []
    const eyebrows: RenderedEye[] = []
    const nose: RenderedEye[] = []
    const cheeks: RenderedFaceFeature[] = []
    const mouth: RenderedEye[] = []
    ...
    if (pose.eyeAlpha > 0.01) {
      ...
      for (let i = 0; i < 2; i++) {
        ...
        const eyeX = e.x * fit + (offX + decalage.x) * R
        const eyeY = e.y * fit + (offY + decalage.y) * R
        const alpha = pose.eyeAlpha * clamp(e.depth / 0.12)

        eyes.push({
          d: capsulePath(cfg.w * R * fScale, cfg.h * R * fScale),
          matrix: `matrix(${r2(ax)},${r2(ay * k)},${r2(cx2)},${r2(cy2 * k)},${r2(eyeX)},${r2(eyeY)})`,
          alpha
        })

        // --- Sourcils (Eyebrows) ---
        if (this.eyebrows !== 'none') {
          const browDist = cfg.h * R * fScale * 0.65 + 8 * fScale
          const bx = eyeX - cx2 * browDist
          const by = eyeY - cy2 * browDist
          let browD = ''
          if (this.eyebrows === 'arched') {
            const bw = 14 * fScale
            const bh = 3 * fScale
            browD = `M ${r2(-bw / 2)} ${r2(bh / 2)} Q 0 ${r2(-bh * 1.8)} ${r2(bw / 2)} ${r2(bh / 2)} Q 0 ${r2(-bh * 0.4)} ${r2(-bw / 2)} ${r2(bh / 2)} Z`
          } else if (this.eyebrows === 'straight') {
            browD = capsulePath(13 * fScale, 2.8 * fScale)
          } else if (this.eyebrows === 'dots') {
            browD = capsulePath(3.8 * fScale, 3.8 * fScale)
          }
          if (browD) {
            eyebrows.push({
              d: browD,
              matrix: `matrix(${r2(ax)},${r2(ay)},${r2(cx2)},${r2(cy2)},${r2(bx)},${r2(by)})`,
              alpha
            })
          }
        }
        ...
        // --- Joues / Blush (Cheeks) ---
        if (this.cheeks !== 'none') {
          const cheekDown = cfg.h * R * fScale * 0.5 + 10 * fScale
          const cheekOut = (i === 0 ? -1 : 1) * (14 * fScale)
          const chx = eyeX + cx2 * cheekDown + ax * cheekOut
          const chy = eyeY + cy2 * cheekDown + ay * cheekOut
          if (this.cheeks === 'blush') {
            cheeks.push({
              d: capsulePath(13 * fScale, 6 * fScale),
              matrix: `matrix(${r2(ax)},${r2(ay)},${r2(cx2)},${r2(cy2)},${r2(chx)},${r2(chy)})`,
              color: '#ff6b8b',
              opacity: 0.6 * alpha
            })
          } else if (this.cheeks === 'dots') {
            cheeks.push({
              d: capsulePath(7.5 * fScale, 7.5 * fScale),
              matrix: `matrix(${r2(ax)},${r2(ay)},${r2(cx2)},${r2(cy2)},${r2(chx)},${r2(chy)})`,
              color: '#ff6b8b',
              opacity: 0.6 * alpha
            })
          } else if (this.cheeks === 'lines') {
            const s = fScale
            const linesD = `M ${r2(-4 * s)} ${r2(-3 * s)} L ${r2(-2 * s)} ${r2(3 * s)} M 0 ${r2(-3 * s)} L ${r2(2 * s)} ${r2(3 * s)} M ${r2(4 * s)} ${r2(-3 * s)} L ${r2(6 * s)} ${r2(3 * s)}`
            cheeks.push({
              d: linesD,
              matrix: `matrix(${r2(ax)},${r2(ay)},${r2(cx2)},${r2(cy2)},${r2(chx)},${r2(chy)})`,
              color: 'none',
              stroke: '#ff6b8b',
              strokeWidth: 1.6 * fScale,
              opacity: 0.8 * alpha
            })
          }
        }
      ...
      if (this.nose !== 'none') {
        const e0 = poses[0]!
        const e1 = poses[1]!
        if (e0.depth > 0.02 || e1.depth > 0.02) {
          const midX = (e0.x + e1.x) / 2
          const midY = (e0.y + e1.y) / 2
          const midA = (e0.a + e1.a) / 2
          const midB = (e0.b + e1.b) / 2
          const midC = (e0.c + e1.c) / 2
          const midD = (e0.d + e1.d) / 2
          const fit = bodyRadius(midX, midY)
          const noseDist = 14 * fScale
          const nx = midX * fit + (offX + decalage.x) * R + midC * noseDist
          const ny = midY * fit + (offY + decalage.y) * R + midD * noseDist
          let noseD = ''
          if (this.nose === 'dot') {
            noseD = capsulePath(3.4 * fScale, 3.4 * fScale)
          } else if (this.nose === 'pill') {
            noseD = capsulePath(2.8 * fScale, 6 * fScale)
          } else if (this.nose === 'snout') {
            const sw = 4.2 * fScale
            const sh = 3.2 * fScale
            noseD = `M ${r2(-sw)} ${r2(-sh / 2)} Q 0 ${r2(-sh)} ${r2(sw)} ${r2(-sh / 2)} Q 0 ${r2(sh * 1.2)} ${r2(-sw)} ${r2(-sh / 2)} Z`
          }
          if (noseD) {
            nose.push({
              d: noseD,
              matrix: `matrix(${r2(midA)},${r2(midB)},${r2(midC)},${r2(midD)},${r2(nx)},${r2(ny)})`,
              alpha: pose.eyeAlpha * clamp((e0.depth + e1.depth) / 0.24)
            })
          }
        }
      }
      ...
      if (this.mouth !== 'none') {
        ...
        if (e0.depth > 0.02 || e1.depth > 0.02) {
          ...
          const midD = (e0.d + e1.d) / 2
          const fit = bodyRadius(midX, midY)
          const mouthDist = (this.nose !== 'none' ? 24 : 19) * fScale
          const mx = midX * fit + (offX + decalage.x) * R + midC * mouthDist
          const my = midY * fit + (offY + decalage.y) * R + midD * mouthDist
          let mouthD = ''
          if (this.mouth === 'smile') {
            const mw = 14 * fScale
            const mh = 5.5 * fScale
            const mth = 2.4 * fScale
            mouthD = `M ${r2(-mw / 2)} ${r2(-mh / 2)} Q 0 ${r2(mh)} ${r2(mw / 2)} ${r2(-mh / 2)} Q 0 ${r2(mh - mth)} ${r2(-mw / 2)} ${r2(-mh / 2)} Z`
          } else if (this.mouth === 'grin') {
            const gw = 15 * fScale
            const gh = 8 * fScale
            mouthD = `M ${r2(-gw / 2)} ${r2(-gh / 3)} L ${r2(gw / 2)} ${r2(-gh / 3)} Q 0 ${r2(gh)} ${r2(-gw / 2)} ${r2(-gh / 3)} Z`
          } else if (this.mouth === 'frown') {
            const fw = 13 * fScale
            const fh = 5 * fScale
            const fth = 2.2 * fScale
            mouthD = `M ${r2(-fw / 2)} ${r2(fh / 2)} Q 0 ${r2(-fh)} ${r2(fw / 2)} ${r2(fh / 2)} Q 0 ${r2(-fh + fth)} ${r2(-fw / 2)} ${r2(fh / 2)} Z`
          } else if (this.mouth === 'open') {
            mouthD = capsulePath(7.5 * fScale, 9.5 * fScale)
          } else if (this.mouth === 'cat') {
            const cw = 14 * fScale
            const ch = 3.5 * fScale
            mouthD = `M ${r2(-cw / 2)} ${r2(-ch / 2)} Q ${r2(-cw / 4)} ${r2(ch)} 0 ${r2(-ch / 4)} Q ${r2(cw / 4)} ${r2(ch)} ${r2(cw / 2)} ${r2(-ch / 2)} Q ${r2(cw / 4)} ${r2(ch - 1.8 * fScale)} 0 ${r2(-ch / 4 + 1.2 * fScale)} Q ${r2(-cw / 4)} ${r2(ch - 1.8 * fScale)} ${r2(-cw / 2)} ${r2(-ch / 2)} Z`
          } else if (this.mouth === 'straight') {
            mouthD = capsulePath(12 * fScale, 2.4 * fScale)
          }
          if (mouthD) {
            mouth.push({
              d: mouthD,
              matrix: `matrix(${r2(midA)},${r2(midB)},${r2(midC)},${r2(midD)},${r2(mx)},${r2(my)})`,
              alpha: pose.eyeAlpha * clamp((e0.depth + e1.depth) / 0.24)
            })
          }
        }
