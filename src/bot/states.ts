...
export const STATES: StateDef[] = [
...

  {
    ...
    pose: (t) => {
      const p = clamp(t / 2.4)
      if (p < 0.25) {
        const k = easings.easeOutCubic(p / 0.25)
        return base({
          offY: -0.07 * k,
          gaze: { yaw: 0, pitch: -6 * k, roll: 0 },
          eyes: pair(0.24 + 0.06 * k, 0.46 + 0.08 * k)
        })
      } else if (p < 0.75) {
        const scanP = (p - 0.25) / 0.5
        const glance = Math.sin(scanP * TAU)
        return base({
          offY: -0.07,
          gaze: { yaw: glance * 20, pitch: -6, roll: glance * 4 },
          eyes: pair(0.30, 0.54)
        })
      } else {
        const retP = (p - 0.75) / 0.25
        const k = 1 - easings.easeInOutCubic(retP)
        return base({
          offY: -0.07 * k,
          gaze: { yaw: 0, pitch: -6 * k, roll: 0 },
          eyes: pair(0.24 + 0.06 * k, 0.46 + 0.08 * k)
...

  {
    ...
    pose: (t) => {
      const p = clamp(t / 2.6)
      if (p < 0.25) {
        const k = easings.easeInOutCubic(p / 0.25)
        return base({
          offX: -0.06 * k,
          offY: -0.03 * k,
          gaze: { yaw: -10 * k, pitch: -4 * k, roll: -16 * k },
          eyes: [
            { w: 0.24, h: 0.50, open: 1 },
            { w: 0.22, h: 0.42, open: 1 }
          ]
        })
      } else if (p < 0.45) {
        return base({
        ...
        const shift = (p - 0.45) / 0.3
        const k = easings.easeInOutCubic(shift)
        const roll = -16 + k * 32
        const yaw = -10 + k * 20
        const offX = -0.06 + k * 0.12
        return base({
          offX,
          offY: -0.03,
          gaze: { yaw, pitch: -4, roll },
          eyes: [
            { w: 0.22 + 0.02 * (1 - k), h: 0.42 + 0.08 * (1 - k), open: 1 },
            { w: 0.22 + 0.02 * k, h: 0.42 + 0.08 * k, open: 1 }
          ]
        })
      } else {
        const retP = (p - 0.75) / 0.25
...

  {
    ...
    pose: (t) => {
      const p = clamp(t / 2.4)
      if (p < 0.35) {
        const k = easings.easeInOutCubic(p / 0.35)
        return base({
          offY: 0.10 * k,
          gaze: { yaw: -4 * k, pitch: 16 * k, roll: -6 * k },
          eyes: pair(0.24 - 0.04 * k, 0.46 - 0.24 * k)
        })
      } else if (p < 0.55) {
        return base({
          offY: 0.10,
          gaze: { yaw: -4, pitch: 16, roll: -6 },
          eyes: pair(0.20, 0.22)
        })
      } else if (p < 0.85) {
        ...
        return base({
          offY: 0.10 - 0.14 * k,
          gaze: { yaw: -4 + 6 * k, pitch: 16 - 22 * k, roll: -6 + 8 * k },
          eyes: pair(0.20 + 0.08 * k, 0.22 + 0.30 * k)
        })
      } else {
        const retP = (p - 0.85) / 0.15
        const k = 1 - easings.easeInOutCubic(retP)
        return base({
          offY: 0.04 * k,
          gaze: { yaw: 2 * k, pitch: 4 * k, roll: 2 * k },
          eyes: pair(0.24, 0.46)
        })
      }
    }
...

  {
    ...
    pose: (t) => {
      ...
      if (p < 0.35) {
        const hopP = p / 0.35
        const hop = Math.sin(hopP * Math.PI)
        return base({
          offY: -0.14 * hop,
          gaze: { yaw: 0, pitch: -10 * hop, roll: Math.sin(hopP * TAU) * 4 },
          eyes: pair(0.28 + 0.08 * hop, 0.52 + 0.24 * hop)
        })
      } else if (p < 0.75) {
        const hopP = (p - 0.35) / 0.4
        const hop = Math.sin(hopP * Math.PI)
        return base({
          offY: -0.20 * hop,
          gaze: { yaw: 0, pitch: -14 * hop, roll: -Math.sin(hopP * TAU) * 6 },
          eyes: pair(0.32 + 0.10 * hop, 0.58 + 0.30 * hop)
        })
      } else {
        const settleP = (p - 0.75) / 0.25
        const k = Math.sin(settleP * Math.PI)
        return base({
          offY: -0.04 * k,
          gaze: { yaw: 0, pitch: -4 * k, roll: 0 },
          eyes: pair(0.26 + 0.04 * k, 0.50 + 0.10 * k)
        })
      }
    }
  },

  {
    ...
    pose: (t) => {
      const p = clamp(t / 2.2)
      if (p < 0.75) {
        const nodP = (p / 0.75) * TAU * 2
        const decay = Math.exp(-p * 1.6)
        const wave = Math.sin(nodP) * decay
        return base({
          offY: wave * 0.08,
          gaze: { yaw: 0, pitch: wave * 18, roll: 0 },
          eyes: pair(0.24, 0.46 - wave * 0.06)
        })
      } else {
        return base()
      }
...

  {
    ...
    pose: (t) => {
      const p = clamp(t / 2.4)
      if (p < 0.8) {
        const shakeP = (p / 0.8) * Math.PI * 5
        const decay = Math.exp(-p * 2.2)
        const shake = Math.sin(shakeP) * decay
        return base({
          offX: shake * 0.06,
          gaze: { yaw: shake * 24, pitch: 0, roll: shake * 4 },
          eyes: pair(0.28, 0.20)
        })
      } else {
        const endP = (p - 0.8) / 0.2
        const k = 1 - easings.easeInOutCubic(endP)
        return base({
          eyes: pair(0.24 + 0.04 * k, 0.46 - 0.26 * k)
        })
      }
    }
  },

  {
    ...
    pose: (t) => {
      ...
      if (p < 0.35) {
        ...
        return base({
          offY: -0.12 * hop,
          gaze: { yaw: 6 * hop, pitch: -8 * hop, roll: 8 * hop },
          eyes: pair(0.28 + 0.06 * hop, 0.52 + 0.16 * hop)
        })
      } else if (p < 0.75) {
        const waveP = (p - 0.35) / 0.4
        const wave = Math.sin(waveP * Math.PI * 4) * Math.exp(-waveP * 1.5)
        return base({
          offY: -0.03 * Math.exp(-waveP * 2),
          gaze: { yaw: 8, pitch: -4, roll: wave * 14 },
          eyes: pair(0.28, 0.54)
        })
      } else {
        ...
        const k = 1 - easings.easeInOutCubic(endP)
        return base({
          gaze: { yaw: 8 * k, pitch: -4 * k, roll: 0 },
          eyes: pair(0.24 + 0.04 * k, 0.46 + 0.08 * k)
        })
      }
    }
  },

  {
    ...
    pose: (t) => {
      const p = clamp(t / 3.0)
      if (p < 0.5) {
        const droopP = easings.easeInOutCubic(p / 0.5)
        return base({
          offY: 0.12 * droopP,
          gaze: { yaw: -3 * droopP, pitch: 18 * droopP, roll: 5 * droopP },
          eyes: pair(0.32, 0.46 - 0.38 * droopP)
        })
      } else if (p < 0.75) {
        // Deep snoozing nod
        return base({
          offY: 0.12,
          gaze: { yaw: -3, pitch: 18, roll: 5 },
          eyes: pair(0.32, 0.08)
        })
      } else if (p < 0.90) {
        // Sudden wake-up snap!
        const snapP = (p - 0.75) / 0.15
        const snap = Math.sin(snapP * Math.PI)
        return base({
          offY: 0.12 - 0.20 * snap,
          gaze: { yaw: 0, pitch: 18 - 28 * snap, roll: 5 - 10 * snap },
          eyes: pair(0.24 + 0.14 * snap, 0.08 + 0.60 * snap)
        })
      } else {
        const retP = (p - 0.90) / 0.10
        const k = 1 - easings.easeInOutCubic(retP)
        return base({
          gaze: { yaw: 0, pitch: -4 * k, roll: 0 },
          eyes: pair(0.24 + 0.04 * k, 0.46 + 0.10 * k)
        })
      }
    }
  }
